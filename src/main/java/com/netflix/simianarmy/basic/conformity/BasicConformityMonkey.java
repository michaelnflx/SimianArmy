...
public class BasicConformityMonkey extends ConformityMonkey {
     ...
     */
    private final Collection<Cluster> nonconformingClusters = Lists.newArrayList();

    /**
     * Clusters that are conforming in the last check.
     */
    private final Collection<Cluster> conformingClusters = Lists.newArrayList();

    /**
     * Clusters that the monkey failed to check for some reason.
     */
    private final Collection<Cluster> failedClusters = Lists.newArrayList();

    /**
     * Clusters that do not exist in the cloud anymore.
     */
    private final Collection<Cluster> nonexistentClusters = Lists.newArrayList();
        ...
        if (!isConformityMonkeyEnabled()) {
            ...
            for (Cluster trackedCluster : trackedClusters) {
                if (!existingClusterNames.contains(trackedCluster.getName())) {
                    nonexistentClusters.add(trackedCluster);
                }
            }
            clusterTracker.deleteClusters(nonexistentClusters.toArray(new Cluster[nonexistentClusters.size()]));
    ...
    private void appendSummary(StringBuilder message, String summaryName,
                               Collection<Cluster> clusters, String region, boolean showDetails) {
