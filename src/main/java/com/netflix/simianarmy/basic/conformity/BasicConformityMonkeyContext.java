...
import com.netflix.simianarmy.aws.conformity.rule.InstanceIsHealthyInDiscovery;
...
public class BasicConformityMonkeyContext extends BasicSimianArmyContext implements ConformityMonkey.Context {
    ...
    public BasicConformityMonkeyContext() {
        ...
        boolean discoveryEnabled = configuration().getBoolOrElse("simianarmy.conformity.Eureka.enabled", false);

        if (discoveryEnabled) {
            LOGGER.info("Initializing Discovery client.");
            DiscoveryClient discoveryClient = DiscoveryManager.getInstance().getDiscoveryClient();
            if (configuration().getBoolOrElse(
                    "simianarmy.conformity.rule.InstanceIsHealthyInDiscovery.enabled", false)) {
                ruleEngine.addRule(new InstanceIsHealthyInDiscovery(discoveryClient));
            }
            if (configuration().getBoolOrElse(
                    "simianarmy.conformity.rule.InstanceHasHealthCheckUrl.enabled", false)) {
                ruleEngine.addRule(new InstanceHasHealthCheckUrl(discoveryClient));
            }
            if (configuration().getBoolOrElse(
                    "simianarmy.conformity.rule.InstanceHasStatusUrl.enabled", false)) {
                ruleEngine.addRule(new InstanceHasStatusUrl(discoveryClient));
            }
        } else {
        ...
        defaultEmail = configuration().getStrOrElse("simianarmy.conformity.notification.defaultEmail", "");
        ccEmails = StringUtils.split(
                configuration().getStrOrElse("simianarmy.conformity.notification.ccEmails", ""), ",");
        sourceEmail = configuration().getStrOrElse("simianarmy.conformity.notification.sourceEmail", "");
